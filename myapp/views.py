...
def getData(request):
    ...
    if (request.method == 'POST' and request.is_ajax()):
        ...
        data = Business.objects.filter(keyword=str(request.POST.get('product')).lower().strip(),
                                       city=str(request.POST.get('city')).lower().strip(), country='Germany').order_by(
            'name')
        if (data.count() > 0):
            print(data)
            filename = './' + id + '.csv';
            ...
            for d in data:
                ...
                area = d.area
                df.append((name, rating, details, street, street_number, postalCode, area, city, country, phonenumber,
                           weblink, email))
            dataFrame = pd.DataFrame(df, columns=(
                'Firmenname / COMPANY NAME', 'rating', 'Branche / INDUSTRY', 'Strasse / Street', 'Strasse no',
                "PostalCode", "Area",
            ...
            return JsonResponse({'msg': 'success', 'id': id, 'count': data.count(), 'rating_count': str(rating_count),
                                 'phone_count': str(phone_count), 'email_count': str(email_count),
                                 'website_count': str(website_count),
                                 'name_count': str(name_count), 'postalCode_count': str(postalCode_count)});

        file = scrapper.getData(str(request.POST.get('product')).lower().strip(),
                                str(request.POST.get('city')).lower().strip(), 'Germany', id,
        ...
        if (file['total'] == 0):
            ...
            return JsonResponse(
                {'msg': 'success', 'id': id, 'count': file['total'], 'rating_count': file['rating_count'],
                 'phone_count': file['phone_count'], 'email_count': file['email_count'],
                 'website_count': file['website_count'],
                 'name_count': file['name_count'], 'postalCode_count': file['postalCode_count']});


def finalizeCheckout(request, args):
    charge = int((int(request.session['total']) * 20) / 100)
    tax = float(charge) * 0.19
    totalprice = float(charge) + tax
        ...
        if (response.result.status == 'APPROVED'):
            ...
            if (totalprice == float(response.result.purchase_units[0].amount.value)):
                ...
                if (response.result.status == 'COMPLETED'):
                    ...
                    order_data = Orders(total=request.session['total'], city=request.session['city'],
                                        keyword=request.session['keyword'], net_price=charge, amount=totalprice,
                                        status=response.result.status, orderID=response.result.id,
                                        email=request.POST['email'], address=request.POST['address'],
                                        company_name=request.POST['company'], client_name=request.POST['name'])
...
def getFile(request):
    if ('id' in request.session.keys() and 'filepath' in request.session.keys() and request.session[
        'payment'] == 'yes'):
...
def getPay(request):
    if ('id' in request.session.keys() and 'filepath' in request.session.keys() and request.session[
        ...
        tax = float(charge) * 0.19
        totalprice = float(charge) + tax
        return render(request, 'payment.html', context={'totalprice': totalprice, 'tax': tax,
                                                        'charge': int((int(request.session['total']) * 20) / 100),
                                                        'total': int(request.session['total'])})
    else:
        ...
        if (request.method == 'POST' and 'filepath' in request.session.keys() and request.session['payment'] != 'yes'):
            ...
            if (charge.paid):
                ...
                return render(request, 'payment.html',
                              context={'msg': 'Failed! there was a problem with your payment. Retry!',
                                       'charge': int((int(request.session['total']) * 20) / 100)})
